...
"""
Mega Lucario ex Deck - Improved Agent（デッキ固有ロジック）

このファイルはメガルカリオexデッキに固有のカード評価・戦術を担当する。
カードの性能に基づく汎用的な判断（サイド計算、相手のKO推定、安全リトリート判定など）は
agent_generic.py に切り出している。
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # --- 安全リトリート判定（汎用ロジックに委譲） ---
    # サイドが残り少なく、アクティブ（メガルカリオex等）が倒されると負ける状況で、
    # 相手の攻撃で倒される可能性があり、かつベンチに「より取られるサイドが少ない」ポケモンがいる場合は、
    # 逃げてベンチポケモンを前に出す（攻撃 or 待機でメガルカリオを延命する）。
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                ...
                if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                    if o.playerIndex == my_index:
                        ...
                        # サイドが残り少ない時は、倒されてもサイド1枚のポケモンを優先して前に出し、
                        # 倒されると負けるポケモン（メガルカリオex等）は前に出さない
                        if my_prize <= 3 and isinstance(card, Pokemon):
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Switch:
                    if safety_retreat:
                        # サイド負け回避: エネルギーを失わずにメガルカリオをベンチに下げる
                        score = 9700
                    elif plan.attacker <= 0:
                        score = -1
                    else:
                        score = 6000
                    ...
                    # デッキが5枚未満の場合はCarmineを使わない（デッキ切れエラー防止）
