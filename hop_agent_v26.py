...
# 先読み探索API（Kaggle Linux環境のみ利用可能なCライブラリ経由）
_SEARCH_ENABLED = False
try:
    from cg.api import search_begin as _search_begin, search_step as _search_step, \
...
try:
    ...
    for _a in _all_attack():
        ...
        _atk_damage[_a.attackId] = _a.damage
except Exception:
    BENCH_DAMAGE_ATTACKS = set()


# =====================================================================
# 先読み探索 (Monte Carlo 1ターン先読み)
#
# 仕組み:
#   MAINフェーズで上位候補を最大6つ取り出し、それぞれについて
#   search_begin / search_step でシミュレーションを回す。
#     - 自分ターンの残り行動: _fast_policy (攻撃優先の簡易戦略)
#     - 相手ターン:           _fast_policy (相手も攻撃優先で動かす)
#   1ターン先(ターン番号が+2になるまで)の局面を _eval_state で評価し、
#   スコアが高い候補に最大+500のボーナスを加える。
#
# 制限事項:
#   - 相手デッキは自分デッキのコピーで代用 (未知のため)
#   - search_begin_input が None の場合 (Macローカル等) は無効化
#   - 例外時はボーナス0で通常スコアにフォールバック
# =====================================================================

def _fast_policy(obs: Observation) -> list[int]:
    """シミュレーション中の簡易意思決定。
    MAIN: 攻撃(高ダメ) > 進化 > エネ付与 > 終了。
    その他コンテキスト: 先頭N枚を選択。
    """
    sel = obs.select
    ctx = sel.context
    ...
    if ctx == SelectContext.MAIN:
        best_dmg, best_atk = -1, -1
        evolve_i = attach_i = end_i = -1
        for i, o in enumerate(opts):
            if o.type == OptionType.ATTACK:
                dmg = _atk_damage.get(o.attackId, 50)
                if dmg > best_dmg:
                    best_dmg, best_atk = dmg, i
            elif o.type == OptionType.EVOLVE and evolve_i < 0:
                evolve_i = i
            elif o.type == OptionType.ATTACH and attach_i < 0:
                attach_i = i
    ...
    n = max(sel.minCount, 1)
    return list(range(min(n, len(opts))))


def _eval_state(state, mi: int) -> int:
    """局面評価値 (大きいほど player mi に有利)。"""
    if state.result == mi:
        return 100000
    if state.result != -1:
        return -100000
    ...
    # 残サイド差: 相手の方が多い=有利
    score = (len(op.prize) - len(my.prize)) * 1500

    my_active = my.active[0] if my.active else None
    op_active = op.active[0] if op.active else None
    if my_active:
        score += my_active.hp            # アクティブのHP残量(耐久指標)
        score += len(my_active.energies) * 40
    if op_active:
        score -= op_active.hp            # 相手アクティブのHP削り量

    for p in my.bench:
        if p is None:
            continue
        if p.id == Hop_Trevenant:
            score += 200 if len(p.energies) >= 1 else 80   # Revenge圏内
...
def _search_score(obs_raw: dict, action_idx: int) -> int:
    """action_idx を選んだ後1ターン先をシミュレーションして局面評価値を返す。
    失敗時(ライブラリ未対応/例外)はニュートラル値 0 を返す。
    """
    ...
    try:
        ...
        if obs.search_begin_input is None:
            return 0  # ネスト検索は不可 (探索状態から呼ばれた場合)
        state = obs.current
        mi = state.yourIndex
        initial_turn = state.turn
        we_go_first = (state.firstPlayer == mi)

        my = state.players[mi]
        op  = state.players[1 - mi]

        # --- 非公開情報の推測 ---
        # 自分のデッキ残り枚数に合わせてサブセット
        my_deck_guess  = my_deck[:max(my.deckCount, 0)]
        my_prize_n     = len(my.prize)
        my_prize_guess = my_deck[:my_prize_n]

        # 相手デッキ: 自分デッキの先頭を使い回す(未知のため最も簡単な仮定)
        op_total = op.deckCount + len(op.prize) + op.handCount
        if op_total > 0:
            reps = op_total // 60 + 2
            opp_pool = (my_deck * reps)[:op_total]
        else:
            opp_pool = []
        opp_deck_guess  = opp_pool[:op.deckCount]
        opp_prize_guess = opp_pool[op.deckCount: op.deckCount + len(op.prize)]
        opp_hand_guess  = opp_pool[op.deckCount + len(op.prize):
                                   op.deckCount + len(op.prize) + op.handCount]

        # 相手アクティブが伏せ状態の場合、有効なBasicポケモンIDを推測
        opp_active_guess: list[int] = []
        if op.active and op.active[0] is None:
            for cid in opp_deck_guess:
                cd = card_table.get(cid)
                if cd and cd.basic and cd.cardType == CardType.POKEMON:
                    opp_active_guess = [cid]
                    break

        ss = _search_begin(obs, my_deck_guess, my_prize_guess,
                           opp_deck_guess, opp_prize_guess, opp_hand_guess,
                           opp_active_guess)
        ss = _search_step(ss.searchId, [action_idx])
        search_id = ss.searchId

        for _ in range(80):
            s_obs   = ss.observation
            s_state = s_obs.current

            # ゲーム終了
            if s_state.result != -1:
                val = 100000 if s_state.result == mi else -100000
                _search_release(search_id)
                return val

            # 1ターン先（自分ターン + 相手ターン終了）まで読んで打ち切り
            if s_state.turn > initial_turn + 1:
                val = _eval_state(s_state, mi)
                _search_release(search_id)
                return val

            if s_obs.select is None:
                break

            choices = _fast_policy(s_obs)
            ss = _search_step(search_id, choices)

        val = _eval_state(ss.observation.current, mi)
        _search_release(search_id)
        return val
...
def _search_refine(obs_raw: dict, scores: list[int]) -> None:
    """MAINフェーズの scores を先読み結果で補正する（in-place）。
    上位6候補をシミュレーションし、相対優劣に応じて最大+500のボーナスを付与する。
    """
    if not _SEARCH_ENABLED:
        return
    ...
    candidates = [i for i in indexed[:6] if scores[i] >= 0]
    if len(candidates) < 2:
        return

    sim_vals = {i: _search_score(obs_raw, i) for i in candidates}
    vals = list(sim_vals.values())
    ...
    for idx, val in sim_vals.items():
        norm = (val - lo) / spread        # 0.0〜1.0 に正規化
        scores[idx] += int(norm * 500)    # 最大 +500 のボーナス
...
def _agent_impl(obs_dict: dict) -> list[int]:
    ...
    # ベンチにRevenge起動可能なオーロットがいるか（アクティブ交代の判断用）。
    # アクティブがオーロット以外で攻撃手段が弱い場合（ボクレー10点等）に退場して
    # ベンチのオーロットを前に出す判断に使う。
    bench_trev_ready = any(
        p is not None and p.id == Hop_Trevenant
        and attack_feasible(p, Trevenant_Revenge, any(t.id == Hop_Choice_Band for t in p.tools))
        for p in my_state.bench
    )
            ...
            for atk in ATTACKS_OF.get(my_active.id, []):
                ...
                for j, opp in enumerate(op_cards):
                    ...
                    if opp.hp <= dmg:
                        ...
                        # [FIX] hop_ko_last_turn=Trueならこのターンのオーロットのリベンジは確実に+100。
                        # Falseの時のみ「不確定」として基礎ダメージ(30)で保守的に評価する。
                        # v23以前の「常にFalse(30点)」はRevenge発動済みでも130点と評価できていなかった。
                        dmg_chip = attack_damage(my_active, atk, opp, is_active, op_prize,
                                                 total_bench, boost, fairy_zone, hop_ko_last_turn)
    ...
    def energy_score(p: Pokemon, active: bool) -> int:
        # エネルギー付与優先順位:
        #   1.オーロット 2.ウッウ(サイド3/4圏内のみ) 3.ピッピ(終盤フィニッシュ時)
        #   4.カビゴン(悪相手のみ) 5.ウールー/バイウールー
        #
        # ★横展開戦略: オーロット/ボクレー/ウッウは1エネで攻撃可能。
        #   1エネあれば十分なので1体に集めず複数アタッカーに分散する。
        #   KOされても次のオーロットが即Revenge(+100=130点)で攻撃できる盤面を維持する。
        nene = len(p.energies)
        score = 8000
        if active:
            score += 30
        if p.id == Hop_Trevenant:
            score += 150 if nene == 0 else 5              # 1番目。1エネあれば横展開
        elif p.id == Hop_Phantump:
            # アクティブのボクレーのみ1エネ優先（殴り要員）。ベンチは進化要員で不要。
            if active:
                score += 150 if nene == 0 else 5
            else:
                score += 20 if nene == 0 else -50
        elif p.id == Hop_Cramorant:
            # 2番目。きまぐれスピットはサイド3/4でしか火力が出ない。圏外は強くゲート。
            if op_prize in (3, 4):
                score += 130 if nene == 0 else 5          # 1エネあればFickle起動。横展開OK
            else:
                score -= 3000   # 圏外では付けない（v23の-200から強化: 無駄なエネ消費を防ぐ）
        elif p.id == Lillie_Clefairy_ex:
            # 終盤フィニッシュ(want_clefairy)時は最優先でフルムーンロンド分を集める。準備は1エネまで。
            if want_clefairy or (op_prize <= 3 and op_has_dragon):
                score += 150
            elif len(p.energies) >= 1:
                score -= 2000
            else:
                score += 110                              # 3番目の優先度で1個だけ準備
        elif p.id == Hop_Snorlax:
            # 4番目。悪相手のみアタッカー解禁（それ以外はベンチ常駐でエネ不要）。
            score += 90 if op_has_dark else -200
        elif p.id == Hop_Wooloo:
            score += 70                                   # 5番目
        elif p.id == Hop_Dubwool:
            score += 80                                   # ウールーの進化先（5番目のライン）
        elif p.id == Genesect:
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if safety_retreat:
                ...
                score = 2000
            elif (bench_trev_ready
                  and active_pk is not None
                  and active_pk.id not in (Hop_Trevenant, Hop_Dubwool)):
                # [FIX] アクティブがオーロット以外で、ベンチに攻撃可能なオーロットがいる。
                # ボクレー(10点)/ウールー(50点)が前に出ている間、Revenge130点を撃てるオーロットが
                # ベンチで待機している状況。退場してオーロットを前に出す。
                score = 2200
            else:
                score = -1
            ...
            if o.attackId == plan.attack_id and plan.attacker == 0:
                if plan.remain_hp <= 0:
                    score = 3500   # KO確定: 最優先
                elif hop_ko_last_turn and o.attackId == Trevenant_Revenge:
                    # Revenge発動確定(130点)。KOはできないが確実な大ダメージを与える。
                    score = 2000
                elif plan.remain_hp < 50:
                    score = 1800   # あと1-2発でKO圏内
                else:
                    score = 1500   # チップダメージ
            else:
                score = 700  # プラン外の攻撃
    ...
    # MAINフェーズのみ先読み補正を適用。サイド1〜5枚の局面で有効。
    if context == SelectContext.MAIN and 0 < op_prize < 6:
        _search_refine(obs_dict, scores)
